...
const addPickup = catchAsync(async (req, res, next) => {
  ...
  const riderWithUpdatedCurrentTour = response?.data.riders.find(rider => rider.updateCurrentTour === true);
...
const deletePickup = catchAsync(async (req, res, next) => {
  ...
  // Formatting request body
  const requestBody = { ...formatRequestBodyToAddPickup(riders, orders, depot), delOrderId: orderId };
